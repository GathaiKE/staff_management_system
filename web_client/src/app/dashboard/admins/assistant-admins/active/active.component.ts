...
@Component({
  selector: 'app-active',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, NgFor, FormComponent, PaginatorComponent, MatTooltipModule, SpinnerComponent],
  ...
  fetchData() {
    ...
    this.authService.currentUser$.subscribe(res => {
      ...
      this.staffServce.staffData$.subscribe(users =>{
        this.admins = users.filter(user=>user.is_assistant_superadmin && user.is_active)
        this.unfilteredAdmins = users.filter(user=>user.is_assistant_superadmin && user.is_active)
        this.staff = users
        this.pageSize = 10
