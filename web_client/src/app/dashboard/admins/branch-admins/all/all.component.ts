  ...
  fetchData() {
    ...
    this.authService.currentUser$.subscribe(res => {
      ...
      this.staffServce.staffData$.subscribe(users =>{
        this.admins = users.filter(user=>user.is_branchadmin && user.user_organization.organization.id === this.currentUser.user_organization.organization.id)
        this.unfilteredAdmins = users.filter(user=>user.is_branchadmin && user.user_organization.organization.id === this.currentUser.user_organization.organization.id)
        this.staff = users
        this.pageSize = 10
