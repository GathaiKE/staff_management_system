...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, NgFor, FormsModule, PaginatorComponent, NgIf, SpinnerComponent],
  templateUrl: './disapproved.component.html',
  styleUrl: './disapproved.component.css'
})
export class DisapprovedComponent {
    ...
    this.authService.currentUser$.subscribe(res => {
      if (res?.id) {
        ...
        this.leaveService.leaveApplications$.subscribe(apps=>{
          this.applications = apps?.filter(a=>a.application.user.user_organization.organization.id === this.organizationId && a.status.toLowerCase().trim() === 'rejected') as LeavesResponse[]
          this.pageSize = 10
